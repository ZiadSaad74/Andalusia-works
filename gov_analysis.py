...
with st.expander("Display the data 🗂️"):
    st.dataframe(df_filtered)    fig2 = px.pie(result_counts, names='Result', values='Count', title= f"Perecentge of each result in ({location_selection}) <br><b>Total leads = {len(df_filtered)} <b>")
